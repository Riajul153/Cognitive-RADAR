...
from src.agents.oracle_mpc_agent import OracleMPCAgent
...
def evaluate_oracle_mpc(n_episodes=50):
    ...
    print(f"Evaluating privileged ORACLE-MPC for {n_episodes} episodes...")
    ...
    for ep in range(n_episodes):
        obs, info = env.reset()
        # Oracle MPC is stateless (only depends on true env state), so no reset needed
        ...
        # We need the true current beam direction for the MPC solver
        # Extract from env
        
        while not (done or truncated):
            current_beam = (env.beam_theta_cmd, env.beam_phi_cmd)
            action = agent.act(current_beam)
